...
function insertAfterSectionContaining(html, needles, block) {
...
const pages = [
  ['index.html', ['Weddings & Events','Weddings &amp; Events','Celebrate'], mediaBlock({ kicker:'Real celebrations', title:'This is Elkhorn.', copy:'Actual Elkhorn wedding photography brings the celebration story back to the property itself—ceremony, portraits and reception details in one connected destination.' })],
  [join('weddings','index.html'), ['See the setting','Imagine the flow'], mediaBlock({ dark:true, kicker:'Real weddings at Elkhorn', title:'Picture the day here.', copy:'Use real moments to build confidence before guests ever reach a package, capacity chart or planning form.' })],
  [join('events','spaces','index.html'), ['Business & Groups','Business &amp; Groups','Celebrations'], mediaBlock({ kicker:'The setting in use', title:'From ceremony to reception.', copy:'The strongest venue story is not an empty room—it is seeing the property become part of the occasion.' })]
];

for (const [relative, needles, block] of pages) {
  const file = join(dist, relative);
  let html = await readFile(file, 'utf8');
  html = html.replace('</head>', '<link rel="stylesheet" href="/assets/poc-client-media.css"></head>');
  html = insertAfterSectionContaining(html, needles, block);
